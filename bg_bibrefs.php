...
*/
	...
	define('BG_BIBREFS_VERSION', '0.7');
	define('BG_BIBREFS_URL',     plugins_url('', __FILE__));

    function  bg_bibfers_script_init() {
		if ( !is_admin() ) {
			wp_enqueue_script('bg_bibrefs.js',  BG_BIBREFS_URL.'/js/bg_bibrefs.js', false, BG_BIBREFS_VERSION, true );
		}
    }
	
	function bg_bibfers($content) {
		if ( is_single() || is_page())
			$content = "<div id='bg_bibfers_id'>".$content."</div><script type='text/javascript' src='".BG_BIBREFS_URL."/js/bg_bibrefs.js'></script>";
//			$content = "<div id='bg_bibfers_id'>".$content."</div>";
		return $content;
	}
	
	if ( defined('ABSPATH') && defined('WPINC') ) {
//		add_action( 'wp_enqueue_scripts', 'bg_bibfers_script_init' );
